...
(() => {
  ...
  const escapePanelHtml = value => String(value == null ? '' : value).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
  const Icon = ({
    name,
    size = 18,
    className = ''
  ...
  const CardVisualStage = ({
    ...
    const fallbackText = !sceneEntry?.folder ? '课程目录已进入前端，交互课件会在完成后显示在这里。' : '当前可视化目录已建立，请补齐场景配置或入口脚本。';
    useEffect(() => {
      let cancelled = false;
      let teardown = null;
      ...
      const renderExternalPanelFallback = (reason = '') => {
        ...
        const focusPoints = Array.isArray(guide.focusPoints) && guide.focusPoints.length ? guide.focusPoints : ['围绕当前可视化画面，先观察空间位置、分布差异与动态过程，再用地理概念解释原因。'];
        const tips = Array.isArray(guide.tips) && guide.tips.length ? guide.tips : ['拖拽或缩放左侧画面，比较不同区域、季节或过程的变化。', '把观察到的现象与课本关键词对应起来，形成一句地理解释。'];
        ...
        `;
        renderGeographyPanelHtml?.(panel, html, {
      ...
      if (externalPanelRef?.current) {
        ...
        renderGeographyPanelHtml?.(externalPanelRef.current, '', {
          preserveScroll: false
        });
      }
      ...
      (async () => {
        ...
        try {
          ...
          console.error('[GeographyScene] failed to load scene script', {
            cardId: card?.id,
            folder: sceneEntry?.folder,
            error
          });
        ...
        if (!definition || typeof definition.mount !== 'function') {
          console.error('[GeographyScene] missing scene definition', {
            cardId: card?.id,
            folder: sceneEntry?.folder
          });
        ...
        try {
          ...
          console.error('[GeographyScene] failed to mount scene', {
            cardId: card?.id,
            folder: sceneEntry?.folder,
            error
          });
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "w-full max-w-sm rounded-[18px] border border-yellow-300/15 bg-zinc-950/80 p-4 text-center shadow-[0_24px_70px_rgba(0,0,0,0.45)]"
    }, /*#__PURE__*/React.createElement("div", {
      className: "text-[11px] font-black tracking-[0.22em] text-yellow-200 uppercase"
      ...
      className: "h-full w-2/5 rounded-full bg-yellow-300 shadow-[0_0_18px_rgba(253,224,71,0.50)] animate-pulse"
    })))) : null, sceneStatus === 'fallback' && sceneFailure ? /*#__PURE__*/React.createElement("div", {
      className: "absolute inset-0 z-10 flex items-end justify-center p-4 pointer-events-none"
    }, /*#__PURE__*/React.createElement("div", {
      className: "w-full max-w-xl rounded-[18px] border border-yellow-300/15 bg-zinc-950/70 p-4 shadow-[0_22px_70px_rgba(0,0,0,0.45)] backdrop-blur-xl"
    }, /*#__PURE__*/React.createElement("div", {
      className: "text-[11px] font-black tracking-[0.22em] text-yellow-200 uppercase"
