/*
  Unified subject auth bridge for the geography app.
...
(() => {
  ...
  const UNIFIED_SUBJECT_KEY = 'geography';
  ...
  function normalizeLocalPreviewBootstrap(subjectKey = UNIFIED_SUBJECT_KEY, payload = null) {
    ...
    const user = Object.assign({}, fallback.user, payload.user || {}, { auth_source: 'local-preview' });
    user.subject_capabilities = Object.assign(
      {},
      fallback.user.subject_capabilities,
      user.subject_capabilities || {},
      { enter_shell: true, open_engine: true }
    );

    const subjectAccess = Object.assign(
      {},
      fallback.subject_access,
      payload.subject_access || {},
      {
        subject_key: normalizedSubjectKey,
        access_status: 'active',
    ...
    const resolvedUser = Object.assign({}, fallback.resolved.user, payload.resolved?.user || user, { auth_source: 'local-preview' });
    resolvedUser.subject_capabilities = Object.assign(
      {},
      fallback.user.subject_capabilities,
      resolvedUser.subject_capabilities || {},
      { enter_shell: true, open_engine: true }
    );
    const resolvedSubjectAccess = Object.assign(
      {},
      fallback.resolved.subject_access,
      payload.resolved?.subject_access || subjectAccess,
      {
        subject_key: normalizedSubjectKey,
        access_status: 'active',
  ...
  function getUnifiedSubjectAuthHeaders(extraHeaders = {}) {
    const token = getStoredUnifiedToken();
    return token
      ? Object.assign({}, extraHeaders, { Authorization: `Bearer ${token}` })
  ...
  async function fetchUnifiedSubjectBootstrap(subjectKey = UNIFIED_SUBJECT_KEY) {
    ...
    const response = await fetch(`/api/unified/bootstrap/subjects/${encodeURIComponent(normalizedSubjectKey)}`, {
      ...
      headers: getUnifiedSubjectAuthHeaders({ 'X-SHG-Subject': normalizedSubjectKey })
    });
  ...
  function useUnifiedSubjectBootstrap(subjectKey = UNIFIED_SUBJECT_KEY) {
    const [payload, setPayload] = useState(() => (
      isLocalPreviewHost()
        ? ensureLocalPreviewBootstrap(subjectKey)
        : readUnifiedSubjectBootstrap(subjectKey)
    ));
    ...
    useEffect(() => {
      ...
      const refreshFromApi = () => {
        ...
        refreshPromise = fetchUnifiedSubjectBootstrap(subjectKey)
          .then((nextPayload) => {
  ...
  function UnifiedAccountDock({ subjectKey = UNIFIED_SUBJECT_KEY }) {
    ...
    return (
      <>
        ...
        >
          <div className="text-[9px] font-black uppercase tracking-[0.28em] text-yellow-300/90">统一账号</div>
          <div className="mt-1 text-sm font-black leading-tight">{displayName}</div>
          <div className="mt-1 text-[11px] text-white/55">{membershipTier} · {accessStatus}</div>
        </button>

        {open ? (
          <div className="absolute inset-0 z-[160] flex items-center justify-center bg-black/55 px-6 py-8 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-[30px] border border-white/10 bg-[#07110f]/95 p-6 text-white shadow-[0_24px_80px_rgba(0,0,0,0.36)]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.32em] text-yellow-300/90">PowerTech在线教学演示</div>
                  <div className="mt-2 text-2xl font-black italic tracking-tight">{displayName}</div>
                  <div className="mt-2 text-sm text-white/65">当前通过统一平台进入地理可视化系统</div>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-6 space-y-3">
                <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-3">
                  <div className="text-[10px] font-black uppercase tracking-[0.24em] text-white/40">会员编号</div>
                  <div className="mt-2 text-sm font-semibold text-white/90 break-all">{memberId}</div>
                </div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-3">
                    <div className="text-[10px] font-black uppercase tracking-[0.24em] text-white/40">会员类型</div>
                    <div className="mt-2 text-sm font-semibold text-white/90">{membershipTier}</div>
                  </div>
                  <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-3">
                    <div className="text-[10px] font-black uppercase tracking-[0.24em] text-white/40">学科权限</div>
                    <div className="mt-2 text-sm font-semibold text-white/90">{accessStatus}</div>
                  </div>
                </div>
                <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-3">
                  <div className="text-[10px] font-black uppercase tracking-[0.24em] text-white/40">有效期</div>
                  <div className="mt-2 text-sm font-semibold text-white/90">{expiresText}</div>
                </div>
              </div>
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  ...
                  disabled={loggingOut}
                  className="flex-1 rounded-[20px] bg-yellow-400 px-4 py-3 text-[11px] font-black uppercase tracking-[0.22em] text-black transition-all hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {loggingOut ? '退出中...' : '退出登录'}
                </button>
              </div>
