...
(() => {
  ...
  function CardTile({
    ...
    const titleClass = frame.isPortrait ? len > 12 ? 'text-[11px]' : len > 8 ? 'text-[14px]' : 'text-[18px]' : compact ? len > 12 ? 'text-[14px]' : len > 8 ? 'text-[18px]' : 'text-[22px]' : frame.roomy ? len > 12 ? 'text-[22px]' : len > 8 ? 'text-[28px]' : 'text-[36px]' : len > 12 ? 'text-[17px]' : len > 8 ? 'text-[22px]' : 'text-[28px]';
    ...
    return /*#__PURE__*/React.createElement("button", {
      ...
      className: `font-black uppercase ${frame.isPortrait ? 'text-[8px] tracking-[0.2em]' : 'text-[10px] tracking-[0.4em]'} ${active ? 'text-slate-300' : 'text-slate-500'}`
    }, typeLabel), /*#__PURE__*/React.createElement("div", {
      className: `rounded-full border px-2.5 py-1 text-[9px] font-black tracking-[0.16em] backdrop-blur-md ${isReady ? 'border-emerald-200/20 bg-emerald-300/10 text-emerald-100' : isTopic ? 'border-amber-200/25 bg-amber-300/10 text-amber-100' : isMasterpiece ? 'border-white/15 bg-black/35 text-zinc-300' : 'border-white/10 bg-black/35 text-zinc-400'}`
    }, isMasterpiece ? isReady ? `已整理 ${card.topicCount} 专题` : '规划中' : isReady ? '已接入' : isTopic ? '重点考查' : '待接入')), /*#__PURE__*/React.createElement("h3", {
    ...
    }, card.title), recitationPassage ? /*#__PURE__*/React.createElement("div", {
      className: "cn-card-recitation-status"
    }, "\u80CC\u8BF5 \xB7 ", recitationStatus) : null, !frame.isPortrait && /*#__PURE__*/React.createElement("div", {
      className: `mt-5 flex flex-wrap gap-2 transition-all duration-500 ${active ? 'opacity-100' : 'opacity-90'}`
    }, card.tags.slice(0, 3).map(tag => /*#__PURE__*/React.createElement("span", {
      key: tag,
      className: "rounded-md border border-white/5 bg-white/5 px-2.5 py-1 text-[9px] font-bold text-zinc-300"
    }, "#", tag))), /*#__PURE__*/React.createElement("div", {
      className: `mt-auto flex items-center justify-between pt-5 transition-all duration-500 ${active ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`
    }, /*#__PURE__*/React.createElement("span", {
      className: "text-[10px] font-black tracking-widest text-slate-300"
    }, isMasterpiece && isReady ? '进入书目' : isReady ? '进入课件' : isTopic ? '查看考点' : isMasterpiece ? '查看简介' : '查看卡片'), /*#__PURE__*/React.createElement("span", {
      className: "text-sm font-black text-slate-300"
    }, "\u2192"))), /*#__PURE__*/React.createElement("div", {
      className: `absolute -inset-x-20 top-0 h-px bg-gradient-to-r from-transparent via-slate-400/70 to-transparent transition-transform duration-1000 ${active ? 'translate-x-full' : '-translate-x-full'}`
    }));
  }
  Object.assign(app, {
    CardTile
