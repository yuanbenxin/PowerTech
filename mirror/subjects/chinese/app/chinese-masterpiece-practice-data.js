...
(() => {
    ...
    function buildMasterpieceChoices(question, allQuestions) {
        if (Array.isArray(question.options) && question.options.length === 4) {
            ...
            return question.options
                .map((text, index) => ({ id: `choice-${question.id}-${index}`, text, fullText: text, correct: text === question.answer }))
                .map(choice => ({ choice, order: stableHash(`${seed}:position:${choice.id}`) }))
        ...
        const distractors = candidates
            .map(candidate => ({ candidate, order: stableHash(`${seed}:${candidate.id}`) }))
            .sort((left, right) => left.order - right.order)
            .slice(0, 3)
            .map(({ candidate }) => ({ id: `choice-${candidate.id}`, text: summarizeChoice(candidate.answer), fullText: candidate.answer, correct: false }));
        const choices = [{ id: `choice-${question.id}`, text: summarizeChoice(question.answer), fullText: question.answer, correct: true }, ...distractors];
        return choices
            .map(choice => ({ choice, order: stableHash(`${seed}:position:${choice.id}`) }))
            .sort((left, right) => left.order - right.order)
            .map(({ choice }) => choice);
    ...
    function gradeMasterpieceChoice(choices, selectedId) {
        ...
        return { status: correct ? 'correct' : 'retry', ratio: correct ? 1 : 0, selectedId, correctChoice: choices.find(choice => choice.correct) || null };
    }

    function flattenPracticeQuestions(payload) {
        return (payload?.cards || [])
            .filter(card => card.visualReviewStatus === 'verified')
            .flatMap(card => card.questions.map(question => ({ ...question, card })));
    }
    ...
    function pickPracticeQuestions(payload, progress, count, mode = 'smart') {
        ...
        const pickFrom = preferred => {
            ...
            return candidatePool.map(item => {
                ...
                return { item, score: Math.random() * Math.max(0.15, recencyPenalty) * weaknessBoost * (item.card.importance || 1) };
            }).sort((left, right) => right.score - left.score)[0].item;
        };

        for (let index = 0; index < count && selected.length < all.length; index += 1) {
            let preferred;
            if (mode === 'review') preferred = pools.weak.length ? pools.weak : pools.review;
            else if (index % 5 === 0) preferred = pools.review.length ? pools.review : pools.fresh;
            else if (index % 3 === 0) preferred = pools.weak.length ? pools.weak : pools.fresh;
            else preferred = pools.fresh.length ? pools.fresh : (pools.weak.length ? pools.weak : pools.review);
    ...
    function updateMasterpieceProgress(progress, question, result) {
        const previous = progress?.questions?.[question.id] || { attempts: 0, correct: 0, mastery: 0 };
        ...
        const next = {
            ...
            questions: {
                ...(progress?.questions || {}),
                [question.id]: { attempts, correct, mastery, status: result.status, lastAnsweredAt: Date.now() }
            }
        };
