...
(() => {
  ...
  function getAuthHeaders(extra = {}) {
    ...
    return token ? Object.assign({}, extra, {
  ...
  function writeBootstrap(subjectKey = SUBJECT_KEY, payload = null) {
    ...
    try {
      window.sessionStorage?.setItem(bootstrapKey(subjectKey), JSON.stringify({
        payload,
        cachedAt: Date.now()
      }));
    } catch (error) {}
  ...
  async function requestUnifiedApi(path, options = {}) {
    ...
    if (isLocalPreview()) {
      if (String(path).startsWith('/api/announcements/recent')) return {
        ok: true,
        items: [],
        unread_total: 0
      };
      if (String(path).startsWith('/api/suggestions?')) return {
        ok: true,
        items: [],
        stats: {
          pending: 0,
      ...
      if (path === '/api/suggestions/submit' && method === 'POST') return {
        ok: true,
        item: null
      };
      return {
    ...
    const response = await fetch(path, {
      ...
      cache: 'no-store',
      credentials: 'same-origin',
      headers: getAuthHeaders(Object.assign({
        'X-SHG-Subject': SUBJECT_KEY
      }, options.json ? {
  ...
  async function logoutFromUnifiedSubject(subjectKey = SUBJECT_KEY) {
    if (isLocalPreview()) {
      try {
        ...
        window.sessionStorage?.removeItem(bootstrapKey(subjectKey));
      } catch (error) {}
  ...
  function useUnifiedSubjectBootstrap(subjectKey = SUBJECT_KEY) {
    ...
    useEffect(() => {
      let active = true;
      fetchBootstrap(subjectKey).then(next => {
