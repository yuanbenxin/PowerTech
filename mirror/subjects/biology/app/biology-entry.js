...
(() => {
  ...
  const BIOLOGY_FEEDBACK_TYPES = Object.freeze([
    Object.freeze({
      id: 'bug',
      label: '问题反馈',
  ...
  function buildUnifiedBiologyHref(reason, options = {}) {
    try {
      const baseHref = typeof getUnifiedPortalHref === 'function'
        ? getUnifiedPortalHref(reason, BIOLOGY_SUBJECT_KEY)
  ...
  function BiologyAccessPrompt({ title, description, actionHref, actionLabel }) {
    return (
      <div className="rounded-[24px] border border-emerald-400/14 bg-emerald-500/[0.06] p-5">
        <div className="text-sm font-black tracking-[0.14em] text-emerald-100">{title}</div>
  ...
  function BiologyAnnouncementCenter({ enabled, loginHref }) {
    ...
    if (!enabled) {
      return (
        <section className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
          <div className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-300/85">公告中心</div>
          <div className="mt-3">
            <BiologyAccessPrompt
              title="登录后查看专属公告"
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-[22px] border px-4 py-4 text-left transition-all ${
                      isActive
                        ? 'border-emerald-400/20 bg-emerald-500/[0.10]'
                        : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.05]'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="truncate text-sm font-black text-white">{getBiologyAnnouncementTitle(item)}</div>
                      {!item?.is_read ? (
                    ...
                    <div className="mt-2 line-clamp-2 text-xs leading-6 text-white/56">
                      {getBiologyAnnouncementBody(item)}
                    </div>
            ...
            <div className="rounded-[24px] border border-white/10 bg-[#06110f]/82 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-xl font-black tracking-tight text-white">
                  {getBiologyAnnouncementTitle(selectedAnnouncement)}
                </div>
                <span className={`inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-black tracking-[0.16em] ${
  ...
  function BiologyFeedbackCenter({ enabled, loginHref }) {
    ...
    if (!enabled) {
      return (
        <section className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
          <div className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-300/85">意见箱</div>
          <div className="mt-3">
            <BiologyAccessPrompt
              title="登录后使用意见箱"
          ...
          <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {BIOLOGY_FEEDBACK_TYPES.map(item => (
                  <button
                    key={item.id}
                    type="button"
              ...
              records.map(record => {
                ...
                return (
                  ...
                  >
                    ...
                    {replyText ? (
                      <div className="mt-4 rounded-[18px] border border-emerald-400/14 bg-emerald-500/[0.06] px-4 py-4">
                        <div className="text-[11px] font-black tracking-[0.12em] text-emerald-100">处理回复</div>
      ...
      <div className="absolute inset-0 z-[120] flex items-center justify-center bg-black/60 px-4 py-5 backdrop-blur-md sm:px-6">
        <div className={`w-full ${maxWidth} max-h-full overflow-hidden rounded-[34px] border border-white/10 bg-[#07110f]/96 text-white shadow-[0_30px_120px_rgba(0,0,0,0.45)]`}>
          <div className="flex items-start justify-between gap-5 border-b border-white/8 px-5 py-5 sm:px-7">
            <div className="min-w-0">
              <div className="text-[10px] font-black uppercase tracking-[0.34em] text-emerald-300/90">{eyebrow}</div>
  ...
  function BiologyBetaNotice({ announcement, onClose }) {
    const [submitting, setSubmitting] = useState(false);
  ...
  function App() {
    ...
    const unifiedPayload = typeof useUnifiedSubjectBootstrap === 'function'
      ? useUnifiedSubjectBootstrap(BIOLOGY_SUBJECT_KEY)
      : null;
    ...
    useEffect(() => {
      ...
      (async () => {
        const [course, manifest, engineConfig] = await Promise.all([
          ...
          fetchUnifiedBiologyEngineConfig()
        ]);
    ...
    const activeCoursewareNotice = activeCard
      ? activeCardHasCourseware
        ? coursewareFeedback
        : (coursewareFeedback || (activeCardIsCheckingCourseware ? BIOLOGY_COURSEWARE_CHECKING_MESSAGE : BIOLOGY_COURSEWARE_MISSING_MESSAGE))
      : coursewareFeedback;
    ...
    useEffect(() => {
      ...
      setCoursewareReadiness(current => {
        ...
        changed = Object.keys(next).length !== Object.keys(current).length
          || Object.keys(next).some(key => next[key] !== current[key]);
      ...
      const runWorker = async () => {
        while (!cancelled && queue.length) {
          ...
          setCoursewareReadiness(current => {
            ...
            return { ...current, [cardId]: nextStatus };
          });
        }
      };
    ...
    const activeBackground = resolveLocalMediaKey(
      BOOK_BACKGROUND_MAP[bookId] || (stageId === 'junior' ? 'assets/bg_j.png' : 'assets/bg_b1.png'),
      stageId === 'junior' ? 'assets/bg_j.png' : 'assets/bg_b1.png'
    );
    ...
    const profileHref = useMemo(
      () => buildUnifiedBiologyHref('subject_profile', { tab: 'profile' }),
      []
    );
    ...
    const handleLogout = async () => {
      ...
      try {
        ...
        window.location.href = typeof getUnifiedPortalHref === 'function'
          ? getUnifiedPortalHref('logout', BIOLOGY_SUBJECT_KEY)
          : '/';
      } finally {
        setLoggingOut(false);
      }
      ...
      return (
        <div className="fixed inset-0 bg-[#080808] text-zinc-100 flex items-center justify-center px-6">
          <div className="w-full max-w-2xl rounded-[36px] border border-rose-500/20 bg-black/60 p-10 text-center backdrop-blur-3xl">
            ...
            <div className="text-3xl font-black italic tracking-tight text-white">Biology Curriculum</div>
        ...
        <div className="shg-subject-auth-overlay !bg-[#080808]">
          <div className="shg-subject-auth-card">
            ...
            <div className="shg-subject-auth-meta-row">
              <div className="shg-subject-auth-meta">生物可视化</div>
    ...
    if (coursewareCard && canOpenCourseware) {
      return (
        <>
          ...
          onExit={backToHub}
        />
        <BiologyBetaNotice announcement={betaNotice} onClose={() => setBetaNotice(null)} />
      </>
      );
    }
        ...
        {viewport => {
          ...
          return (
            <div
              ...
              style={{
                background: `radial-gradient(circle at center, rgba(12,18,15,0.7) 0%, rgba(5,5,5,0.95) 100%), url(${resolveBiologyMediaThumbnailPath(activeBackground, 'assets/bg_j.png', { width: 1920 })}) center/cover no-repeat`
              }}
            >
            <header
            ...
            >
              ...
              >
                <div className={`${portraitCompact ? 'w-7 h-7' : 'w-8 h-8'} rounded-md border border-emerald-500/60 bg-emerald-500/10 flex items-center justify-center group-hover:bg-emerald-500/20 group-active:bg-emerald-500/25 transition-all`}>
                  <span className="text-emerald-400 font-black text-[14px] translate-y-[0.5px]">生</span>
                </div>
              ...
              <div className={`flex items-center ${portraitCompact ? 'gap-1.5' : compact ? 'gap-2.5' : 'gap-3.5'}`}>
                ...
                <div className={`${narrowStack ? 'hidden' : 'flex'} items-center gap-2.5 ${headerMetaClass} font-black text-emerald-500`}>
                        ...
                        <div className="flex-1 flex flex-col min-w-0">
                          ...
                          >
                            <div className="w-full h-full">
                              <img
                                src={resolveBiologyMediaThumbnailPath(activeCard.image, activeCard.fallbackImage || 'assets/c1.png', { width: 1280 })}
                        ...
                        <div className="flex flex-col shrink-0" style={{ width: narrowStack ? '100%' : `${detailAsideWidth}px` }}>
                          ...
                          >
                            ...
                            <div className={`${narrowStack ? 'pt-3' : 'pt-6'} border-t border-white/10 mt-auto`}>
                              <h5 className={`text-[9px] font-black text-emerald-400 tracking-widest uppercase ${narrowStack ? 'mb-2' : 'mb-3'}`}>
                                摘要 / Abstract
                              </h5>
                          ...
                          <button
                            ...
                            className={`${narrowStack ? 'hidden' : 'w-full'} font-black text-sm shadow-xl transition-all shrink-0 ${compact ? 'py-4 rounded-xl tracking-widest' : 'py-5 rounded-2xl tracking-[0.2em]'} ${
                              activeCardHasCourseware
                                ? 'bg-emerald-500 text-black shadow-emerald-500/20 hover:scale-[1.02] active:scale-[0.985] active:bg-emerald-400 active:shadow-emerald-500/35'
                        ...
                        <div className="shrink-0 border-t border-white/10 bg-black/78 p-3 backdrop-blur-2xl">
                          ...
                          <button
                            ...
                            className={`w-full min-h-[44px] rounded-xl text-sm font-black tracking-widest shadow-xl transition-all ${
                              activeCardHasCourseware
                                ? 'bg-emerald-500 text-black shadow-emerald-500/25 active:scale-[0.985] active:bg-emerald-400'
                                : 'cursor-not-allowed border border-white/10 bg-white/[0.06] text-white/35 shadow-black/10'
                            }`}
                            style={{ touchAction: 'manipulation', WebkitTapHighlightColor: 'transparent' }}
                          >
              ...
              >
                <div className="space-y-4">
                  <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
                    <div className="rounded-[30px] border border-white/10 bg-white/[0.04] p-5">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          <div className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-300/85">会员资料</div>
                          <div className="mt-3 text-3xl font-black italic tracking-tight text-white">{displayName}</div>
                          <div className="mt-3 flex flex-wrap items-center gap-2">
                            <span className="inline-flex items-center rounded-full border border-emerald-400/18 bg-emerald-500/[0.08] px-3 py-1 text-[11px] font-black tracking-[0.16em] text-emerald-100">
                              {membershipTier}
                            </span>
                      ...
                      <div className="mt-5 rounded-[24px] border border-emerald-500/12 bg-emerald-500/[0.08] px-4 py-4 text-sm leading-7 text-white/78">
                        {hasAuthorizedAccount
                          ? '当前账号已进入生物可视化工作台，你可以在这里接收公告并提交使用建议。'
                    ...
                    <div className="space-y-4">
                      <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-5">
                        ...
                        <div className="mt-2 text-base font-semibold text-emerald-300">{currentBookLabel}</div>
                  ...
                  <div className="grid gap-4 xl:grid-cols-2">
                    <BiologyAnnouncementCenter enabled={hasAuthorizedAccount} loginHref={profileHref} />
                    <BiologyFeedbackCenter enabled={hasAuthorizedAccount} loginHref={profileHref} />
                  </div>
                </div>
              </DashboardOverlayShell>
            ) : null}
