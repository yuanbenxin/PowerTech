...
(() => {
  ...
  const BIOLOGY_MEDIA_BASE = normalizeRemoteMediaBase(BIOLOGY_RUNTIME.mediaBase ?? BIOLOGY_RUNTIME.MEDIA_BASE ?? DEFAULT_BIOLOGY_MEDIA_BASE);
  const BIOLOGY_COURSEWARE_IMAGE_BASE = normalizeRemoteMediaBase(BIOLOGY_RUNTIME.coursewareImageBase ?? BIOLOGY_RUNTIME.COURSEWARE_IMAGE_BASE ?? `${BIOLOGY_MEDIA_BASE}courseware-images/`);
  ...
  function installCoursewareImageFallback() {
    ...
    window.addEventListener('error', event => {
      ...
      const source = tagName === 'img' ? String(target.currentSrc || target.getAttribute('src') || '') : tagName === 'image' ? String(target.getAttribute('href') || target.getAttributeNS?.('http://www.w3.org/1999/xlink', 'href') || '') : '';
  ...
  function resolveBiologyModelVariantSource(source, options = {}) {
    ...
    const explicitVariants = typeof source === 'object' && source ? source : {
      desktop: source,
    ...
    const ranked = tier === 'mobile' ? [{
      src: explicitVariants.mobile,
      explicit: sourceIsObject || Boolean(options.mobileSrc || options.mobile)
    }, {
      src: explicitVariants.tablet,
      explicit: sourceIsObject || Boolean(options.tabletSrc || options.tablet)
    }, {
      src: desktopSource,
      desktop: true
    }] : tier === 'tablet' ? [{
      ...
      src: explicitVariants.desktop || explicitVariants.desktopSrc || desktopSource,
      desktop: true
    }, {
      src: desktopSource,
      desktop: true
  ...
  function waitForBiologyModelViewerDefinition(timeoutMs = 7000) {
    ...
    return Promise.race([window.customElements.whenDefined('model-viewer').then(() => true), new Promise(resolve => {
      window.setTimeout(() => {
  ...
  function loadBiologyModelViewerScript(source) {
    return new Promise(resolve => {
      ...
      const existingScript = Array.from(document.querySelectorAll('script[data-bio-model-viewer-loader="true"]')).find(script => script.src === resolvedSource);
      if (existingScript) {
  ...
  function buildPanelScrollSelector(root, node) {
    ...
    const stableClasses = Array.from(node.classList || []).filter(className => !/^(?:is-|active|selected|correct|wrong|open|visible|complete|error)/.test(className));
  ...
  function captureBiologyPanelScroll(panel) {
    ...
    return nodes.filter(node => Number(node.scrollTop) > 0 || Number(node.scrollLeft) > 0).map(node => ({
      isRoot: node === panel,
  ...
  function installBiologyPanelScrollPreserver(panel) {
    ...
    applyBiologyPanelScrollLayout(panel);
    const observer = typeof MutationObserver !== 'undefined' ? new MutationObserver(() => applyBiologyPanelScrollLayout(panel)) : null;
    observer?.observe(panel, {
      childList: true
    });
    const descriptor = getNativeInnerHtmlDescriptor();
    if (!descriptor?.get || !descriptor?.set) {
      panel[PANEL_SCROLL_PRESERVER_KEY] = {
        observer
      };
      return;
    }
  ...
  function enhanceBiologyModelViewerProgress(root = document) {
    ...
    viewers.forEach(viewer => {
      ...
      srcObserver.observe(viewer, {
        attributes: true,
        attributeFilter: ['src']
      });
      viewer[MODEL_VIEWER_PROGRESS_KEY] = {
        overlay,
        showProgress,
        hideProgress,
        srcObserver
      ...
      viewer.addEventListener('progress', event => {
        ...
        if (progress >= 1) {
          if (viewer.modelIsVisible === true) hideProgress();else scheduleCompleteFallback();
        }
      });
  ...
  function installBiologyModelViewerProgressObserver() {
    ...
    modelViewerProgressObserver.observe(document.documentElement, {
      childList: true,
      subtree: true
    });
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', installBiologyModelViewerProgressObserver, {
      once: true
    });
  } else {
