...
(() => {
  ...
  async function loadStageQuestionBank(stageId) {
    ...
    const request = loadStageQuestionBankFresh(normalizedStage)
      .then(payload => {
  ...
  async function loadStageQuestionBankFresh(normalizedStage) {
    ...
    if (!indexPath) {
      return { stageId: normalizedStage, label: '题库', questions: [], filters: createEmptyFilters() };
    }
  ...
  function createEmptyFilters() {
    return { grades: [], knowledgePoints: [], types: [], difficulties: [], sourceTypes: [], learningStages: [] };
  }
  ...
  function getQuestionSourceTypeLabel(question) {
    ...
    if (sourceType === 'exam_real') {
      return normalizeText(question?.stageId) === 'senior' || /高考|GAOKAO/i.test(`${question?.source?.name || ''} ${question?.source?.origin || ''} ${question?.source?.paper || ''}`)
        ? '高考真题'
        : '中考真题';
    }
  ...
  function shuffleBrowseQuestions(questions, seed = '') {
    return (questions || [])
      .map((question, index) => ({ question, key: deterministicDrawKey(question.id, index, seed || 'browse') }))
  ...
  function drawQuestions(questions, options = {}) {
    ...
    const candidates = filtered
      .map((question, index) => ({ question, key: deterministicDrawKey(question.id, index, options.seed || '') }))
      .sort((left, right) => left.key.localeCompare(right.key));

    return selectDiverseQuestions(candidates, limit, options)
      .map(item => item.question);
  }
    ...
    if (value && typeof value === 'object') {
      ...
      return Object.fromEntries(
        Object.entries(value)
          .map(([key, nextValue]) => [normalizeText(key), normalizeText(nextValue).replace(/\s+/g, ' ').toUpperCase()])
  ...
  function gradeQuestionAnswer(question, rawAnswer) {
    ...
    if (isMultiChoiceQuestion(question)) {
      ...
      return userAnswer.length === correctAnswer.length && userAnswer.every((label, index) => label === correctAnswer[index])
        ? 'correct'
        : 'wrong';
    }
  ...
  function buildPracticeReport({ questions = [], answers = {} } = {}) {
    const items = questions.map((question, index) => {
      ...
      return {
        index: index + 1,
        question,
        answer: stepResults.length
          ? formatStepAnswerSummary(question.steps, normalizedAnswer)
          : isMultiChoiceQuestion(question)
            ? formatChoiceLabels(rawAnswer)
