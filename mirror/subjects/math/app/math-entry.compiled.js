...
(() => {
  ...
  function isMathLocalPreviewHost() {
    ...
    return isMathLocalStaticHost();
  }
  async function createLocalMathApiPayload(path) {
  ...
  function normalizeMathStageId(value) {
    const normalized = String(value || '').trim().toLowerCase();
    return normalized === 'senior' ? 'senior' : normalized === 'junior' ? 'junior' : '';
  }
  function getInitialMathStageId() {
    try {
      const params = new URLSearchParams(window.location.search || '');
      const stageFromUrl = normalizeMathStageId(params.get('stage'));
      if (stageFromUrl) {
        window.sessionStorage?.setItem(MATH_STAGE_STORAGE_KEY, stageFromUrl);
        return stageFromUrl;
      }
    } catch (error) {}
    return '';
  }
  const MATH_ACCESS_MODE_LABELS = Object.freeze({
    full: '完整访问',
    shell: '试用浏览',
    expired: '授权已到期',
    locked: '待开通'
  });
  const MATH_FEEDBACK_TYPES = Object.freeze([Object.freeze({
    id: 'bug',
    label: '问题反馈',
    ...
    id: 'feature',
    label: '功能建议',
    buttonClass: 'border-slate-400/18 bg-slate-500/[0.08] text-slate-100',
    badgeClass: 'border-slate-400/18 bg-slate-500/[0.08] text-slate-100'
  }), Object.freeze({
    id: 'ui',
    label: '界面优化',
  ...
  function resolveMathReturnPath() {
    try {
      const currentUrl = new URL(window.location.href);
      if (String(currentUrl.protocol || '').toLowerCase() === 'file:') {
        return '/subjects/math/';
      }
      const pathname = String(currentUrl.pathname || '').trim();
      const search = String(currentUrl.search || '').trim();
      if (!pathname) return '/subjects/math/';
      return `${pathname}${search}` || '/subjects/math/';
    } catch (error) {
      return '/subjects/math/';
    }
  }
  function buildMathSubscriptionHref() {
    try {
      const subscriptionPath = isMathLocalStaticHost() ? '/dist-subscription/index.html' : '/subscription/';
      const url = new URL(subscriptionPath, window.location.href);
      url.searchParams.set('subject', MATH_SUBJECT_KEY);
      url.searchParams.set('return', resolveMathReturnPath());
      return url.toString();
    } catch (error) {
      return `/subscription/?subject=${encodeURIComponent(MATH_SUBJECT_KEY)}&return=${encodeURIComponent('/subjects/math/')}`;
    }
  }
  function openMathSubscription() {
    window.location.assign(buildMathSubscriptionHref());
  }
  function buildUnifiedMathHref(reason, options = {}) {
    try {
      const baseHref = typeof getUnifiedPortalHref === 'function' ? getUnifiedPortalHref(reason, MATH_SUBJECT_KEY) : new URL('/', window.location.href).toString();
  ...
  async function fetchUnifiedMathEngineConfig() {
    if (isMathLocalPreviewHost()) {
      return {};
    }
    try {
      const headers = Object.assign({
        'X-SHG-Subject': MATH_SUBJECT_KEY
      }, typeof getUnifiedSubjectAuthHeaders === 'function' ? getUnifiedSubjectAuthHeaders() || {} : {});
      const response = await fetch('/api/unified/subjects/math/engines', {
        method: 'GET',
        cache: 'no-store',
  ...
  function createMathSubjectHeaders(extraHeaders = {}, includeJsonContentType = false) {
  ...
  async function requestMathUnifiedApi(path, options = {}) {
    if (isMathLocalPreviewHost()) {
      const localPayload = await createLocalMathApiPayload(path, options);
    ...
    const response = await fetch(path, {
      ...
      credentials: 'same-origin',
      headers: createMathSubjectHeaders(options.headers || {}, options.json !== undefined),
  ...
  function getMathFeedbackTypeMeta(type) {
    return MATH_FEEDBACK_TYPES.find(item => item.id === type) || MATH_FEEDBACK_TYPES[MATH_FEEDBACK_TYPES.length - 1];
  }
  function getMathFeedbackStatusMeta(status) {
    const normalized = String(status || '').trim().toLowerCase();
    if (normalized === 'adopted') {
      return {
        label: '已采纳',
        className: 'border-slate-400/18 bg-slate-500/[0.08] text-slate-100'
      };
    }
  ...
  function MathAccessPrompt({
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "rounded-[24px] border border-slate-400/14 bg-slate-500/[0.06] p-5"
    }, /*#__PURE__*/React.createElement("div", {
      className: "text-sm font-black tracking-[0.14em] text-slate-100"
    }, title), /*#__PURE__*/React.createElement("div", {
      className: "mt-3 text-sm leading-7 text-white/72"
    }, description), /*#__PURE__*/React.createElement("a", {
      href: actionHref,
      className: "mt-5 inline-flex items-center justify-center gap-2 rounded-[18px] border border-slate-300/18 bg-slate-400 px-4 py-3 text-sm font-black tracking-[0.12em] text-black transition-all hover:brightness-105"
    }, /*#__PURE__*/React.createElement(Icon, {
      name: "ExternalLink",
      size: 15
    }), " ", actionLabel));
  }
  function MathAnnouncementCenter({
    enabled,
    loginHref
  }) {
    ...
    const loadAnnouncements = async (showLoading = true) => {
      ...
      try {
        const payload = await requestMathUnifiedApi('/api/announcements/recent?limit=12&scan=120');
    ...
    const handleSelectAnnouncement = async announcement => {
      ...
      try {
        await requestMathUnifiedApi(`/api/announcements/${encodeURIComponent(announcement.id)}/read`, {
          method: 'POST',
          json: {}
        });
    ...
    if (!enabled) {
      return /*#__PURE__*/React.createElement("section", {
        ...
        className: "mt-3"
      }, /*#__PURE__*/React.createElement(MathAccessPrompt, {
        title: "\u767B\u5F55\u540E\u67E5\u770B\u4E13\u5C5E\u516C\u544A",
    ...
    return /*#__PURE__*/React.createElement("section", {
      ...
      className: `inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-black tracking-[0.16em] ${unreadTotal > 0 ? 'border-slate-400/18 bg-slate-500/[0.08] text-slate-100' : 'border-white/10 bg-white/[0.05] text-white/70'}`
      ...
      return /*#__PURE__*/React.createElement("button", {
        key: String(item?.id || getMathAnnouncementTitle(item)),
        type: "button",
        onClick: () => {
          void handleSelectAnnouncement(item);
        },
        className: `w-full rounded-[22px] border px-4 py-4 text-left transition-all ${isActive ? 'border-slate-400/20 bg-slate-500/[0.10]' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.05]'}`
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-center justify-between gap-3"
      }, /*#__PURE__*/React.createElement("div", {
        className: "truncate text-sm font-black text-white"
      }, getMathAnnouncementTitle(item)), !item?.is_read ? /*#__PURE__*/React.createElement("span", {
        className: "inline-flex h-2.5 w-2.5 rounded-full bg-slate-300 shadow-[0_0_12px_rgba(252,211,77,0.65)]"
      }) : null), /*#__PURE__*/React.createElement("div", {
        className: "mt-2 line-clamp-2 text-xs leading-6 text-white/56"
      }, getMathAnnouncementBody(item)), /*#__PURE__*/React.createElement("div", {
    ...
    }, getMathAnnouncementTitle(selectedAnnouncement)), /*#__PURE__*/React.createElement("span", {
      className: `inline-flex items-center rounded-full border px-3 py-1 text-[11px] font-black tracking-[0.16em] ${selectedAnnouncement?.is_read ? 'border-white/10 bg-white/[0.05] text-white/70' : 'border-slate-400/18 bg-slate-500/[0.08] text-slate-100'}`
    ...
    }, getMathAnnouncementBody(selectedAnnouncement)))));
  }
  function MathFeedbackCenter({
    enabled,
    loginHref
  }) {
    ...
    const selectedType = useMemo(() => getMathFeedbackTypeMeta(type), [type]);
    const loadRecords = async () => {
      if (!enabled) return;
      setLoading(true);
      try {
        const payload = await requestMathUnifiedApi('/api/suggestions?limit=50');
    ...
    const handleSubmit = async event => {
      ...
      setSubmitting(true);
      try {
        await requestMathUnifiedApi('/api/suggestions/submit', {
          method: 'POST',
          json: {
            source: MATH_SUBJECT_KEY,
            type,
            title: title.trim(),
    ...
    return /*#__PURE__*/React.createElement("section", {
      ...
      className: `rounded-[14px] px-4 py-2 text-sm font-black transition-all ${activeSection === item.key ? 'bg-slate-400 text-black' : 'text-white/62 hover:text-white'}`
    }, item.label))), feedbackText ? /*#__PURE__*/React.createElement("div", {
      className: `mt-4 rounded-[20px] border px-4 py-4 text-sm leading-7 ${feedbackTone === 'success' ? 'border-slate-400/18 bg-slate-500/[0.08] text-slate-50' : feedbackTone === 'error' ? 'border-rose-400/18 bg-rose-500/[0.08] text-rose-50' : 'border-white/10 bg-white/[0.04] text-white/76'}`
    ...
    }, MATH_FEEDBACK_TYPES.map(item => /*#__PURE__*/React.createElement("button", {
      key: item.id,
      type: "button",
      ...
      className: "w-full rounded-[18px] border border-white/10 bg-[#07110f] px-4 py-3 text-sm text-white caret-slate-300 outline-none transition-all placeholder:text-white/35 focus:border-slate-400/40 focus:bg-[#0a1814]"
    })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      className: "mb-3 text-xs font-black tracking-[0.16em] text-white/42"
      ...
      className: "min-h-[150px] w-full rounded-[18px] border border-white/10 bg-[#07110f] px-4 py-3 text-sm leading-7 text-white caret-slate-300 outline-none transition-all placeholder:text-white/35 focus:border-slate-400/40 focus:bg-[#0a1814]"
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex flex-wrap items-center justify-between gap-3"
    }, /*#__PURE__*/React.createElement("div", {
      ...
      type: "submit",
      disabled: submitting,
      className: "inline-flex items-center justify-center gap-2 rounded-[18px] bg-slate-400 px-5 py-3 text-sm font-black tracking-[0.14em] text-black transition-all hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60"
    }, /*#__PURE__*/React.createElement(Icon, {
      ...
      const typeMeta = getMathFeedbackTypeMeta(record?.type);
      const statusMeta = getMathFeedbackStatusMeta(record?.status);
      const replyText = getMathSuggestionReply(record);
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "mt-4 rounded-[18px] border border-slate-400/14 bg-slate-500/[0.06] px-4 py-4"
      }, /*#__PURE__*/React.createElement("div", {
        className: "text-[11px] font-black tracking-[0.12em] text-slate-100"
  ...
  function App() {
    ...
    const initialStageId = getInitialMathStageId();
    ...
    const [questionBankReturnQuestionId, setQuestionBankReturnQuestionId] = useState('');
    const [questionBankReturnSnapshot, setQuestionBankReturnSnapshot] = useState(null);
    ...
    const landingBackgroundPool = useMemo(() => unique(['assets/media/bg_j.webp', 'assets/media/bg_s.webp', ...Object.values(BOOK_BACKGROUND_MAP || {})]), []);
    const [landingImageBackground, setLandingImageBackground] = useState(() => {
      const pool = unique(['assets/media/bg_j.webp', 'assets/media/bg_s.webp', ...Object.values(BOOK_BACKGROUND_MAP || {})]);
      return pool[Math.floor(Math.random() * pool.length)] || 'assets/media/bg_j.webp';
    });
    useEffect(() => {
      let active = true;
      async function loadActiveAnnouncement() {
        try {
          const payload = await requestMathUnifiedApi('/api/announcements/active');
    ...
    const unifiedPayload = typeof useUnifiedSubjectBootstrap === 'function' ? useUnifiedSubjectBootstrap(MATH_SUBJECT_KEY) : null;
    ...
    useEffect(() => {
      ...
      const backgroundVideo = document.querySelector('[data-math-landing-video]');
    ...
    useEffect(() => {
      let ignore = false;
      (async () => {
        const [course, manifest, engineConfig] = await Promise.all([loadCourseData(), fetchJsonSafe(VISUAL_MANIFEST_PATH), fetchUnifiedMathEngineConfig()]);
        if (ignore) return;
        if (!course) {
    ...
    const activeCardHasCourseware = Boolean(activeCardSceneEntry?.folder);
    const activeCoursewareNotice = activeCard && !activeCardHasCourseware ? coursewareFeedback || '该卡片的课件资源还没有接入完成，请先选择其他可用课件。' : coursewareFeedback;
    useEffect(() => {
      if (!coursewareCardId || canOpenCourseware) return;
      setCoursewareCardId(null);
      openMathSubscription();
    }, [coursewareCardId, canOpenCourseware]);
    useEffect(() => {
      if (!coursewareCardId || !visualManifest || coursewareSceneEntry?.folder) return;
      setActiveCardId(coursewareCardId);
      setCoursewareCardId(null);
      setCoursewareFeedback('该卡片的课件资源还没有接入完成，请先选择其他可用课件。');
    }, [coursewareCardId, visualManifest, coursewareSceneEntry]);
    useEffect(() => {
      setCoursewareFeedback('');
    }, [stageId, bookId]);
    useEffect(() => {
      setQuestionBankOpen(false);
      setQuestionBankReturnQuestionId('');
      setQuestionBankReturnSnapshot(null);
    }, [stageId]);
    const activeBackground = resolveLocalMediaKey(BOOK_BACKGROUND_MAP[bookId] || (stageId === 'junior' ? 'assets/media/bg_j.webp' : 'assets/media/bg_s.webp'), stageId === 'junior' ? 'assets/media/bg_j.webp' : 'assets/media/bg_s.webp');
    ...
    const currentStageLabel = stage?.label || '数学学科';
    ...
    const accessMode = MATH_ACCESS_MODE_LABELS[String(unifiedPayload?.access_mode || unifiedPayload?.capabilities?.access_mode || '').trim().toLowerCase()] || '同步中';
    const profileHref = useMemo(() => buildUnifiedMathHref('subject_profile', {
      tab: 'profile'
    }), []);
    ...
    const returnQuestionBankCourseware = () => {
      returnCoursewareToList();
      if (questionBankReturnQuestionId) {
    ...
    const enterStage = nextStageId => {
      ...
      try {
        window.sessionStorage?.setItem(MATH_STAGE_STORAGE_KEY, nextStageId);
      } catch (error) {}
      resetDashboardState();
      setView('dashboard');
    };
    ...
    const exitSubjectSystem = () => {
      try {
        window.sessionStorage?.removeItem(MATH_STAGE_STORAGE_KEY);
      } catch (error) {}
    ...
    const openOverlay = nextOverlayMode => setOverlayMode(nextOverlayMode);
    const requestCoursewareOpen = cardId => {
      const targetCard = cards.find(card => card.id === cardId);
      if (!targetCard || targetCard.isLocked) return;
      const targetSceneEntry = sceneEntryMap.get(cardId);
      if (!targetSceneEntry?.folder) {
        setCoursewareCardId(null);
    ...
    const openQuestionBankCard = (cardId, questionId, returnSnapshot) => {
      const normalizedCardId = String(cardId || '').trim();
      if (!normalizedCardId) return;
      const targetCard = cardMap.get(normalizedCardId);
      const targetSceneEntry = sceneEntryMap.get(normalizedCardId);
      if (!targetCard || targetCard.isLocked || !targetSceneEntry?.folder) {
        setCoursewareFeedback('该卡片的课件资源还没有接入完成，请先选择其他可用课件。');
        return;
      }
      setQuestionBankReturnQuestionId(String(questionId || '').trim());
      setQuestionBankReturnSnapshot(returnSnapshot && typeof returnSnapshot === 'object' ? returnSnapshot : null);
      setQuestionBankOpen(false);
      closeOverlay();
      setCoursewareFeedback('');
      requestCoursewareOpen(normalizedCardId);
    };
    const handleCopyMemberId = async () => {
      if (!unifiedUser?.member_id) return;
      const copied = await copyMathText(unifiedUser.member_id);
    ...
    const handleLogout = async () => {
      ...
      try {
        if (typeof logoutFromUnifiedSubject === 'function') {
          await logoutFromUnifiedSubject(MATH_SUBJECT_KEY);
          return;
        }
        const authHeaders = createMathSubjectHeaders();
        try {
        ...
        try {
          window.sessionStorage?.removeItem(`shg:subject-bootstrap:${MATH_SUBJECT_KEY}`);
        } catch (error) {}
        window.location.href = typeof getUnifiedPortalHref === 'function' ? getUnifiedPortalHref('logout', MATH_SUBJECT_KEY) : '/';
      } finally {
        setLoggingOut(false);
      }
    };
    if (loadError) {
      ...
      }, "Math Curriculum"), /*#__PURE__*/React.createElement("div", {
    ...
    if (!courseData) {
      ...
      }, "\u6570\u5B66\u53EF\u89C6\u5316"), /*#__PURE__*/React.createElement("div", {
      ...
      }, "\u6743\u9650\u6838\u9A8C\u5DF2\u901A\u8FC7\uFF0C\u6B63\u5728\u521D\u59CB\u5316\u6570\u5B66\u8BFE\u7A0B\u67B6\u6784\u4E0E\u53EF\u89C6\u5316\u8D44\u6E90\uFF0C\u8BF7\u7A0D\u5019\u3002"), /*#__PURE__*/React.createElement("div", {
    ...
    if (coursewareCard && canOpenCourseware) {
      return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(CoursewareWorkbench, {
        ...
        onBack: returnQuestionBankCourseware,
        onExit: backToHub
      }), questionBankReturnQuestionId ? /*#__PURE__*/React.createElement("button", {
        type: "button",
        "aria-label": "\u8FD4\u56DE\u9898\u5E93\u7EC3\u4E60",
        onClick: returnQuestionBankCourseware,
        className: "fixed z-[85] rounded-full border border-amber-200/40 bg-amber-300 px-5 py-3 text-xs font-black text-black shadow-2xl shadow-amber-500/25 transition-all hover:scale-[1.03] active:scale-95",
        style: {
          right: 'max(20px, env(safe-area-inset-right))',
          bottom: 'max(20px, env(safe-area-inset-bottom))'
        }
      }, "\u8FD4\u56DE\u9898\u5E93") : null, /*#__PURE__*/React.createElement(MathBetaNotice, {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(LandscapeOnlyViewport, {
      title: activeCard?.title || stage?.label || 'THE MATH CURRICULUM',
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          background: `radial-gradient(circle at center, rgba(12,18,15,0.7) 0%, rgba(5,5,5,0.95) 100%), url(${resolveMathMediaThumbnailPath(activeBackground, 'assets/media/bg_j.webp', {
            width: 1920
        ...
        type: "button",
        className: `flex items-center ${portraitCompact ? 'gap-2' : 'gap-3'} cursor-pointer group appearance-none bg-transparent text-left transition-all active:scale-[0.97] active:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300/80`,
        onClick: () => {
          resetDashboardState();
        },
        style: {
        ...
        className: `${portraitCompact ? 'w-7 h-7' : 'w-8 h-8'} rounded-md border border-slate-500/60 bg-slate-500/10 flex items-center justify-center group-hover:bg-slate-500/20 group-active:bg-slate-500/25 transition-all`
      }, /*#__PURE__*/React.createElement("span", {
        className: "text-slate-400 font-black text-[14px] translate-y-[0.5px]"
      }, "\u6570")), /*#__PURE__*/React.createElement("span", {
        ...
        className: `${narrowStack ? 'hidden' : 'flex'} items-center gap-2.5 ${headerMetaClass} font-black text-slate-500`
        ...
        className: `flex w-full max-w-full min-w-0 overflow-x-auto overflow-y-hidden no-scrollbar ${portraitCompact ? 'gap-1.5 px-0.5 pb-0.5' : compact ? 'gap-2' : 'gap-2'}`,
        style: {
        ...
        className: `shrink-0 font-black ${filterChipTrackingClass} rounded-full border transition-all whitespace-nowrap active:scale-95 ${filterChipClass} ${bookId === 'all' ? 'bg-slate-500 text-black border-slate-500 active:bg-slate-400' : 'bg-white/5 border-white/10 text-zinc-400 hover:border-white/20 active:border-white/25 active:text-white active:bg-white/10'}`,
        style: {
          touchAction: 'manipulation',
          WebkitTapHighlightColor: 'transparent'
        }
        ...
        className: `shrink-0 font-black ${filterChipTrackingClass} rounded-full border transition-all whitespace-nowrap active:scale-95 ${filterChipClass} ${bookId === bookItem.id ? 'bg-slate-500 text-black border-slate-500 active:bg-slate-400' : 'bg-white/5 border-white/10 text-zinc-400 hover:border-white/20 active:border-white/25 active:text-white active:bg-white/10'}`,
        style: {
          touchAction: 'manipulation',
          WebkitTapHighlightColor: 'transparent'
        }
        ...
        className: `font-black italic tracking-tighter text-slate-500 ${detailTitleMarginClass} uppercase break-words`,
        ...
        src: resolveMathMediaThumbnailPath(activeCard.image, activeCard.fallbackImage || 'assets/media/math_card_bg_platinum.webp', {
          width: 1280
        }),
        onError: event => handleMathMediaError(event, activeCard.fallbackImage || 'assets/media/math_card_bg_platinum.webp'),
        ...
        className: "w-1.5 h-1.5 bg-slate-500 rounded-full mt-1.5 shrink-0"
      }), point))), /*#__PURE__*/React.createElement("div", {
        className: `${narrowStack ? 'pt-3' : 'pt-6'} border-t border-white/10 mt-auto`
      }, /*#__PURE__*/React.createElement("h5", {
        className: `text-[9px] font-black text-slate-400 tracking-widest uppercase ${narrowStack ? 'mb-2' : 'mb-3'}`
        ...
        className: `${narrowStack ? 'hidden' : 'w-full'} font-black text-sm shadow-xl transition-all shrink-0 ${compact ? 'py-4 rounded-xl tracking-widest' : 'py-5 rounded-2xl tracking-[0.2em]'} ${activeCardHasCourseware ? 'bg-slate-500 text-black shadow-slate-500/20 hover:scale-[1.02] active:scale-[0.985] active:bg-slate-400 active:shadow-slate-500/35' : 'cursor-not-allowed border border-white/10 bg-white/[0.06] text-white/35 shadow-black/10'}`
        ...
        onClick: () => requestCoursewareOpen(activeCard.id),
        disabled: !activeCardHasCourseware,
        className: `w-full min-h-[44px] rounded-xl text-sm font-black tracking-widest shadow-xl transition-all ${activeCardHasCourseware ? 'bg-slate-500 text-black shadow-slate-500/25 active:scale-[0.985] active:bg-slate-400' : 'cursor-not-allowed border border-white/10 bg-white/[0.06] text-white/35 shadow-black/10'}`,
        style: {
          touchAction: 'manipulation',
          WebkitTapHighlightColor: 'transparent'
        }
      }, activeCardHasCourseware ? '进入交互课件' : '课件接入中')) : null)))), null, !activeCard && !coursewareCardId && QuestionBankEntryButton ? /*#__PURE__*/React.createElement(QuestionBankEntryButton, {
    ...
    }), questionBankOpen && QuestionBankWorkspace ? /*#__PURE__*/React.createElement(QuestionBankWorkspace, {
      stageId: stageId,
      stageLabel: currentStageLabel,
      cardMap: cardMap,
      sceneEntryMap: sceneEntryMap,
      returnQuestionId: questionBankReturnQuestionId,
      returnSnapshot: questionBankReturnSnapshot,
      onClose: () => setQuestionBankOpen(false),
      onOpenCard: openQuestionBankCard
    }) : null, /*#__PURE__*/React.createElement(MathBetaNotice, {
      announcement: betaNotice,
      onClose: () => setBetaNotice(null)
    }));
  }
