...
(() => {
  ...
  const JUNIOR_HUD_CARD_IDS = new Set(['j7a_m01', 'j7a_m02', 'j7a_m03', 'j7a_m04', 'j7a_m05', 'j7a_m10', 'j7a_m11', 'j7a_m12', 'j7a_m13', 'j7b_m01', 'j7b_m02', 'j7b_m03', 'j7b_m05', 'j7b_m06', 'j7b_m07', 'j7b_m09', 'j7b_m10', 'j7b_m11', 'j7b_m12', 'j7b_m14', 'j8a_m05', 'j8a_m06', 'j8a_m07', 'j8a_m10', 'j8a_m11', 'j8a_m12', 'j8a_m14', 'j8b_m02', 'j8b_m05', 'j8b_m06', 'j8b_m07', 'j8b_m08', 'j8b_m09', 'j9a_m05', 'j9b_m02', 'j9b_m04', 'jm_model_m03', 'jm_model_m04', 'jm_model_m05', 'jm_model_m06', 'jm_model_m07', 'jm_model_m08', 'jm_model_m09', 'jm_model_m10', 'jm_model_m11', 'jm_model_m13', 'jm_model_m14', 'jm_model_m17', 'jm_model_m18', 'jm_model_m19', 'jm_model_m21', 'jm_model_m23', 'jm_model_m24', 'jm_topic_m03', 'jm_topic_m04', 'jm_topic_m05', 'jm_topic_m06', 'jm_topic_m07', 'jm_topic_m08', 'jm_topic_m09', 'jm_topic_m10', 'jm_topic_m11', 'jm_topic_m12', 'jm_topic_m13', 'jm_topic_m14', 'jm_topic_m15', 'jm_topic_m16', 'jm_topic_m17']);
  ...
  const installUnifiedHudTouchFallback = host => {
    if (!host) return () => {};
    const hudSelector = ['#analysis-hud-card', '.analysis-hud-card', '.analysis-hud', '#hud-chalkboard-panel', '#hud-panel', '.hud-panel', '.hud-board', '.collapsible-hud', '#area-proof-hud', '.area-proof-hud', '.hud-card', '[data-hud]'].join(',');
    const triggerSelector = ['#btn-hud-toggle', '#hud-toggle-btn', '.btn-hud-toggle', '.hud-control-btn', '.hud-toggle', '[data-hud-toggle]', '.hud-header', '.area-proof-hud-header'].join(',');
    let lastToggleAt = 0;
    ...
    const rectContains = (rect, point) => rect && point && point.x >= rect.left && point.x <= rect.right && point.y >= rect.top && point.y <= rect.bottom;
    ...
    const wireHudToggle = hud => {
      ...
      const trigger = triggers.find(node => node.matches?.('button, [role="button"], [data-hud-toggle], .hud-control-btn, .btn-hud-toggle, .hud-toggle')) || triggers[0];
    ...
    const findHudHit = event => {
      ...
      for (const hud of getVisibleHuds()) {
        ...
        const triggers = queryWithin(hud, triggerSelector);
        const preferredTrigger = triggers.find(trigger => trigger.matches?.('button, [role="button"], [data-hud-toggle], .hud-control-btn, .btn-hud-toggle, .hud-toggle')) || triggers[0] || hud;
        const matchedTriggers = triggers.filter(trigger => rectContains(trigger.getBoundingClientRect(), point));
        if (matchedTriggers.length) {
          const interactiveTrigger = matchedTriggers.find(trigger => trigger.matches?.('button, [role="button"], [data-hud-toggle], .hud-control-btn, .btn-hud-toggle, .hud-toggle'));
          return {
            hud,
            trigger: interactiveTrigger || matchedTriggers[0]
          };
        }
        const rect = hud.getBoundingClientRect();
        const topBar = {
    ...
    const runToggle = hit => {
      ...
      const readCollapsed = () => Boolean(hud && (hud.classList.contains('collapsed') || hud.getAttribute('aria-expanded') === 'false'));
      const syncHudState = () => {
        if (!hud) return;
    ...
    const observer = typeof MutationObserver !== 'undefined' ? new MutationObserver(() => getVisibleHuds().forEach(normalizeHudNode)) : null;
  ...
  const CardVisualStage = ({
    ...
    useEffect(() => {
      ...
      const showSceneLoadIssue = message => {
        if (externalPanelRef?.current) {
          externalPanelRef.current.innerHTML = `
            <div style="height:100%;display:flex;align-items:center;justify-content:center;padding:18px;color:#fca5a5;font:800 13px/1.7 Inter,'Microsoft YaHei UI',sans-serif;text-align:left;">
              <div style="width:100%;border:1px solid rgba(248,113,113,.24);background:rgba(127,29,29,.18);border-radius:16px;padding:14px;">
                ${message}
              </div>
            </div>
          `;
      ...
      (async () => {
        ...
        try {
          ...
          if (!cancelled) {
            showSceneLoadIssue('课件引擎脚本没有加载成功。请确认服务器已完整上传 dist-math/visualizations/engines 目录，并清理旧缓存后重试。');
            setSceneStatus('fallback');
          }
          return;
        }
        ...
        try {
          ...
          showSceneLoadIssue('课件源码执行失败。请打开浏览器控制台查看具体脚本错误，或检查对应卡片 source.html 是否已完整上传。');
          if (!cancelled) setSceneStatus('fallback');
        }
      })();
      return () => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      ref: hostRef,
      className: `absolute inset-0 transition-opacity duration-500 ${sceneStatus === 'fallback' ? 'opacity-0' : 'opacity-100'}`
      ...
      className: "w-full max-w-sm rounded-[18px] border border-slate-300/15 bg-zinc-950/80 p-4 text-center shadow-[0_24px_70px_rgba(0,0,0,0.45)]"
    }, /*#__PURE__*/React.createElement("div", {
      className: "text-[11px] font-black tracking-[0.22em] text-slate-200 uppercase"
      ...
      className: "h-full w-2/5 rounded-full bg-slate-300 shadow-[0_0_18px_rgba(203,213,225,0.55)] animate-pulse"
    })))) : null);
  };
