...
(function () {
  const CARD_ID = "jm_model_m04";
  const STYLE_ID = "math-half-angle-model-style";
  const FALLBACK_ERROR = "半角模型课件载入失败，请检查本卡片目录内 source.html、style.css?v=3e89d997541a 与 scene.js?v=2bf76fd99006";
  ...
  function baseStyleText() {
    return `
      ...
      *::after {
      ...
      .half-angle-model-panel-scroll::-webkit-scrollbar {
        width: 0;
        height: 0;
      }
      .control-column {
      ...
      .hud-panel.expanded {
        max-height: min(420px, calc(100% - 44px)) !important;
      ...
      .hud-body {
        max-height: calc(min(230px, calc(100vh - 180px)) - 46px) !important;
        overflow-y: auto !important;
      ...
      /* 说明性信息归入 HUD；舞台仅保留模型、必要标注和缩放工具。 */
      #hud-chalkboard-panel > .teaching-status-card {
      ...
      #hud-chalkboard-panel > .teaching-status-card .status-eyebrow {
        font-size: 9px !important;
      }
      #hud-chalkboard-panel > .teaching-status-card strong {
        font-size: 15px !important;
      }
      #hud-chalkboard-panel > .teaching-status-card #status-detail {
        font-size: 10px !important;
      }
      #hud-chalkboard-panel > #section-theory-card {
      ...
      #hud-chalkboard-panel > #section-theory-card h3 {
        margin: 0 0 7px !important;
        color: #0f172a !important;
        font-size: 12px !important;
      }
      #hud-chalkboard-panel > #section-theory-card .proof-flow {
        display: grid !important;
        gap: 5px !important;
      }
      #hud-chalkboard-panel > #section-theory-card .proof-chip,
      #hud-chalkboard-panel > #section-theory-card .proof-result {
        min-width: 0 !important;
      ...
      #hud-chalkboard-panel > #section-theory-card .proof-chip span,
      #hud-chalkboard-panel > #section-theory-card .proof-result span {
        font-size: 9px !important;
      }
      #hud-chalkboard-panel > #section-theory-card .proof-chip span {
      ...
      #hud-chalkboard-panel > #section-theory-card .proof-chip strong {
        color: #0f172a !important;
      ...
      #hud-chalkboard-panel > #section-theory-card .proof-result strong {
        color: #78350f !important;
        font-size: 10.5px !important;
        font-weight: 800 !important;
      }
      .control-column .preset-desc {
        display: none !important;
      }
      .control-column .btn-preset {
  ...
  function unifiedJmModelHudResponsiveStyle() {
    return `
      /* HUD_RESPONSIVE_SCALE: preserve the chalkboard hierarchy on narrow screens. */
      @media (max-width: 600px) {
        ...
        #hud-chalkboard-panel .hud-row { margin-bottom: 5px !important; padding-bottom: 4px !important; }
        #hud-chalkboard-panel .hud-row-label { font-size: 10px !important; }
        #hud-chalkboard-panel .hud-row-val,
        #hud-chalkboard-panel .hud-equation-box,
        #hud-chalkboard-panel .hud-proof-line,
        #hud-chalkboard-panel .hud-kpi,
        #hud-chalkboard-panel .hud-proof { font-size: 10.5px !important; line-height: 1.3 !important; }
      }
      @media (max-width: 420px) {
        ...
        [data-hud] {
          ...
          max-height: min(300px, calc(100% - 20px)) !important;
        }
        #hud-chalkboard-panel.hud-panel.collapsed,
        #hud-panel.hud-panel.collapsed,
        #hud-panel.collapsible-hud.collapsed,
        ...
        #hud-chalkboard-panel .hud-header > span,
        #hud-panel .hud-header > span,
        .hud-panel .hud-header > span { font-size: 11px !important; }
        #hud-chalkboard-panel .hud-control-btn,
        #hud-panel .hud-control-btn,
        .hud-panel .hud-control-btn,
        .hud-board .hud-control-btn,
...
${unifiedJmModelHudFinalOverrides()}\n${unifiedJmModelHudResponsiveStyle()}`;
  ...
  function extractLayout(body, sourceStyle, cleanup) {
    ...
    const hudPanel = canvasNode.querySelector("#hud-chalkboard-panel");
    const teachingStatusNode = canvasNode.querySelector("#teaching-status-card");
    const theoryNode = panelNode.querySelector("#section-theory-card");
    if (canvasNode.id === "canvas-container") canvasNode.className = "";
    if (panelNode.id === "control-panel") panelNode.className = "";

    // Keep the instruction chain close to the model, while the side panel stays
    // focused on changing the teaching state.
    if (hudPanel) {
      if (teachingStatusNode) hudPanel.appendChild(teachingStatusNode);
      if (theoryNode) hudPanel.appendChild(theoryNode);
    }

    const scene = makeShadowHost("half-angle-model-scene");
    ...
    const panel = makeShadowHost("half-angle-model-panel");
    const panelShell = document.createElement("div");
    panelShell.className = "half-angle-model-panel-scroll";
  ...
  function runSourceScript(script, sceneRoot, panelRoot, sceneHost, cleanup) {
    ...
    const runner = new Function(
      ...
      "clearInterval",
      "CSS",
      `"use strict";\n${source}\n//# sourceURL=jm_model_m04_source.js`
    );
    runner(
      scopedDocument,
      scopedWindow,
  ...
  window.MATH_VISUAL_SCENES[CARD_ID] = {
    async mount(container, context) {
      ...
      try {
        ...
        console.error("Failed to mount half-angle model card:", error);
