...
(function () {
  const CARD_ID = "jm_model_m14";
  const STYLE_ID = "math-hidden-circle-model-style";
  const FALLBACK_ERROR = "隐圆模型课件载入失败，请检查本卡片目录内 source.html、style.css?v=54cb3b14153e 与 scene.js?v=3aedeabe41b9";
  ...
  async function collectSource(doc, sourceUrl) {
    ...
    doc.querySelectorAll('style, script, link[rel="stylesheet"], link[as="style"]').forEach(node => node.remove());
  ...
  function baseStyleText() {
    return `
      ...
      *::after {
      ...
      .hidden-circle-model-panel-scroll {
        width: 100%;
        height: 100%;
        min-height: 0;
        overflow-x: hidden;
      ...
      .hidden-circle-model-panel-scroll::-webkit-scrollbar {
        width: 0;
        height: 0;
      }
      .control-column {
      ...
      .btn-secondary.active .layer-step-num {
      ...
      @media (pointer: coarse), (hover: none), (max-width: 768px) {
        ...
        .geo-point-wrapper .geo-point-halo {
          r: 24px !important;
          pointer-events: all !important;
        }
        .hidden-circle-model-panel-scroll .btn-preset,
        .hidden-circle-model-panel-scroll .btn-primary,
        .hidden-circle-model-panel-scroll .btn-secondary,
        .hidden-circle-model-panel-scroll .layer-step-btn,
        .hidden-circle-model-panel-scroll #btn-play-demo,
        .hidden-circle-model-panel-scroll #btn-reset-state,
        .hidden-circle-model-panel-scroll #btn-toggle-snap,
        .hidden-circle-model-panel-scroll #btn-toggle-grid {
          min-height: 44px !important;
          touch-action: manipulation !important;
        }
        .canvas-controls .btn-zoom,
        #btn-zoom-in,
        #btn-zoom-out,
        #btn-zoom-reset {
        ...
        .hidden-circle-model-panel-scroll input[type="range"] {
          min-height: 44px !important;
          touch-action: manipulation !important;
        }
        .hidden-circle-model-panel-scroll input[type="range"]::-webkit-slider-thumb {
...
${unifiedJmModelHudFinalOverrides()}`;
  ...
  function extractLayout(body, sourceStyle, cleanup) {
    const canvasNode = body.querySelector(".simulation-column") || body.querySelector("#sandbox-wrapper");
    const panelNode = body.querySelector(".control-column");
    ...
    const scene = makeShadowHost("hidden-circle-model-scene");
    ...
    const panel = makeShadowHost("hidden-circle-model-panel");
    const panelShell = document.createElement("div");
    panelShell.className = "hidden-circle-model-panel-scroll";
  ...
  function blockNativeTouchMenus(root, cleanup) {
    ...
    cleanup.addListener(root, "touchstart", event => {
      if (event.touches && event.touches.length > 1 && !event.target.closest?.(".control-column")) {
  ...
  function bridgeHudHostToggle(sceneRoot, sceneHost, cleanup) {
    const hudPanel = sceneRoot.querySelector("#hud-chalkboard-panel");
    const toggleButton = sceneRoot.querySelector("#hud-toggle-btn");
    const hudHeader = sceneRoot.querySelector("#hud-chalkboard-panel .hud-header");
    if (!hudPanel || !toggleButton || !hudHeader || !sceneHost) return;
    ...
    const forward = event => {
      ...
      const headerRect = hudHeader.getBoundingClientRect();
      const collapsedRect = hudPanel.getBoundingClientRect();
      if (!inRect(point, headerRect) && !(hudPanel.classList.contains("collapsed") && inRect(point, collapsedRect))) return;
      const now = Date.now();
      if (now - lastForwardAt < 420) {
        event.preventDefault();
    ...
    ["click", "touchend"].forEach(type => {
      cleanup.addListener(sceneHost, type, forward, { passive: false, capture: true });
    });
  }
  ...
  function runSourceScript(script, sceneRoot, panelRoot, sceneHost, cleanup) {
    ...
    const runner = new Function(
      ...
      "clearInterval",
      "CSS",
      `"use strict";\n${source}\n//# sourceURL=jm_model_m14_source.js`
    );
    runner(
      scopedDocument,
      scopedWindow,
  ...
  window.MATH_VISUAL_SCENES[CARD_ID] = {
    async mount(container, context) {
      ...
      try {
        ...
        bridgeHudHostToggle(sceneRoot, sceneHost, cleanup);
        window.dispatchEvent(new Event("resize"));
      } catch (error) {
        console.error("Failed to mount hidden circle model card:", error);
