...
(function () {
  const CARD_ID = "jm_model_m09";
  const STYLE_ID = "math-a-shape-similarity-model-style";
  const FALLBACK_ERROR = "A 字型相似模型课件载入失败，请检查本卡片目录内 source.html、style.css?v=0e08c5cff977 与 scene.js?v=ed485da5fd92";
  ...
  function baseStyleText() {
    return `
      ...
      *::after {
      ...
      .a-shape-similarity-model-panel-scroll {
        width: 100%;
        height: 100%;
        min-height: 0;
        overflow-x: hidden;
      ...
      .a-shape-similarity-model-panel-scroll::-webkit-scrollbar {
        width: 0;
        height: 0;
      }
      .control-column {
...
${unifiedJmModelHudFinalOverrides()}`;
  ...
  function extractLayout(body, sourceStyle, cleanup) {
    ...
    const scene = makeShadowHost("a-shape-similarity-model-scene");
    ...
    const panel = makeShadowHost("a-shape-similarity-model-panel");
    const panelShell = document.createElement("div");
    panelShell.className = "a-shape-similarity-model-panel-scroll";
  ...
  function runSourceScript(script, sceneRoot, panelRoot, sceneHost, cleanup) {
    ...
    const runner = new Function(
      ...
      "clearInterval",
      "CSS",
      `"use strict";\n${source}\n//# sourceURL=jm_model_m09_source.js`
    );
    runner(
      scopedDocument,
      scopedWindow,
  ...
  window.MATH_VISUAL_SCENES[CARD_ID] = {
    async mount(container, context) {
      ...
      try {
        ...
        console.error("Failed to mount A-shape similarity model card:", error);
