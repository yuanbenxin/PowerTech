...
(function () {
  const CARD_ID = "jm_model_m11";
  const STYLE_ID = "math-right-triangle-altitude-model-style";
  const FALLBACK_ERROR = "直角三角形斜边高相似模型课件载入失败，请检查本卡片目录内 source.html、style.css?v=0a759a3852e5 与 scene.js?v=42702a1c7371";
  ...
  function baseStyleText() {
    return `
      ...
      *::after {
      ...
      .right-triangle-altitude-model-panel-scroll {
        width: 100%;
        height: 100%;
        min-height: 0;
        overflow-x: hidden;
      ...
      .right-triangle-altitude-model-panel-scroll::-webkit-scrollbar {
        width: 0;
        height: 0;
      }
      .control-column {
      ...
      .hud-equation-box .formula.formula-stack {
        display: grid !important;
        grid-template-columns: minmax(0, 1fr) !important;
        align-items: stretch !important;
        justify-content: stretch !important;
      }
      .hud-equation-box .formula.formula-stack > strong,
      .hud-equation-box .formula.formula-stack > .ratio-grid {
        min-width: 0 !important;
        width: 100% !important;
      }
      .ratio-grid {
      ...
      .geo-triangle-name.badge-main {
        fill: #2563eb !important;
      }
      .geo-triangle-name.badge-match {
...
${unifiedJmModelHudFinalOverrides()}`;
  ...
  function extractLayout(body, sourceStyle, cleanup) {
    ...
    const scene = makeShadowHost("right-triangle-altitude-model-scene");
    ...
    if (particlesNode) particlesNode.remove();
    ...
    const panel = makeShadowHost("right-triangle-altitude-model-panel");
    const panelShell = document.createElement("div");
    panelShell.className = "right-triangle-altitude-model-panel-scroll";
  ...
  function runSourceScript(script, sceneRoot, panelRoot, sceneHost, cleanup) {
    ...
    const runner = new Function(
      ...
      "clearInterval",
      "CSS",
      `"use strict";\n${source}\n//# sourceURL=jm_model_m11_source.js`
    );
    runner(
      scopedDocument,
      scopedWindow,
  ...
  window.MATH_VISUAL_SCENES[CARD_ID] = {
    async mount(container, context) {
      ...
      try {
        ...
        console.error("Failed to mount right-triangle altitude similarity model card:", error);
