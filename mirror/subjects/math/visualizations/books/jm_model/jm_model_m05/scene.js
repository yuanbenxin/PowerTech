...
(function () {
  const CARD_ID = "jm_model_m05";
  const STYLE_ID = "math-double-median-model-style";
  const FALLBACK_ERROR = "倍长中线模型课件载入失败，请检查本卡片目录内 source.html、style.css?v=fc579cdf8761 与 scene.js?v=5acc6850769b";
  ...
  function baseStyleText() {
    return `
      ...
      *::after {
      ...
      .double-median-model-panel-scroll {
        width: 100%;
        height: 100%;
        min-height: 0;
        overflow-x: hidden;
      ...
      .double-median-model-panel-scroll::-webkit-scrollbar {
        width: 0;
        height: 0;
      }
      .control-column {
...
${unifiedJmModelHudFinalOverrides()}`;
  ...
  function extractLayout(body, sourceStyle, cleanup) {
    ...
    const scene = makeShadowHost("double-median-model-scene");
    ...
    const panel = makeShadowHost("double-median-model-panel");
    const panelShell = document.createElement("div");
    panelShell.className = "double-median-model-panel-scroll";
  ...
  function runSourceScript(script, sceneRoot, panelRoot, sceneHost, cleanup) {
    ...
    const runner = new Function(
      ...
      "clearInterval",
      "CSS",
      `"use strict";\n${source}\n//# sourceURL=jm_model_m05_source.js`
    );
    runner(
      scopedDocument,
      scopedWindow,
  ...
  window.MATH_VISUAL_SCENES[CARD_ID] = {
    async mount(container, context) {
      ...
      try {
        ...
        console.error("Failed to mount double-median model card:", error);
