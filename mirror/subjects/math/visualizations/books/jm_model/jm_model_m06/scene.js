...
(function () {
  const CARD_ID = "jm_model_m06";
  const STYLE_ID = "math-cut-add-segment-model-style";
  const FALLBACK_ERROR = "截长补短模型课件载入失败，请检查本卡片目录内 source.html、style.css?v=a1dfd3252f58 与 scene.js?v=5327139ae6ed";
  ...
  function baseStyleText() {
    return `
      ...
      *::after {
      ...
      .cut-add-segment-model-panel-scroll {
        width: 100%;
        height: 100%;
        min-height: 0;
        overflow-x: hidden;
      ...
      .cut-add-segment-model-panel-scroll::-webkit-scrollbar {
        width: 0;
        height: 0;
      }
      .control-column {
      ...
      @media (pointer: coarse), (hover: none), (max-width: 768px) {
        ...
        .cut-add-segment-model-panel-scroll .btn-preset,
        .cut-add-segment-model-panel-scroll .btn-primary,
        .cut-add-segment-model-panel-scroll .btn-secondary,
        .cut-add-segment-model-panel-scroll .btn-icon,
        .cut-add-segment-model-panel-scroll #btn-play-folding,
        .cut-add-segment-model-panel-scroll #btn-reset-state,
        .cut-add-segment-model-panel-scroll #btn-show-help,
        .cut-add-segment-model-panel-scroll #btn-close-help {
        ...
        .cut-add-segment-model-panel-scroll input[type="range"] {
          min-height: 44px !important;
          touch-action: manipulation !important;
        }
        .cut-add-segment-model-panel-scroll input[type="range"]::-webkit-slider-thumb {
...
${unifiedJmModelHudFinalOverrides()}`;
  ...
  function extractLayout(body, sourceStyle, cleanup) {
    ...
    const scene = makeShadowHost("cut-add-segment-model-scene");
    ...
    const panel = makeShadowHost("cut-add-segment-model-panel");
    const panelShell = document.createElement("div");
    panelShell.className = "cut-add-segment-model-panel-scroll";
  ...
  function runSourceScript(script, sceneRoot, panelRoot, sceneHost, cleanup) {
    ...
    const runner = new Function(
      ...
      "clearInterval",
      "CSS",
      `"use strict";\n${source}\n//# sourceURL=jm_model_m06_source.js`
    );
    runner(
      scopedDocument,
      scopedWindow,
  ...
  window.MATH_VISUAL_SCENES[CARD_ID] = {
    async mount(container, context) {
      ...
      try {
        ...
        console.error("Failed to mount cut-add segment model card:", error);
