...
(function () {
  const CARD_ID = "jm_model_m13";
  const STYLE_ID = "math-power-of-circle-model-style";
  const FALLBACK_ERROR = "圆幂模型课件载入失败，请检查本卡片目录内 source.html、style.css?v=60bc038e823b 与 scene.js?v=133dc41c2b0c";
  ...
  function baseStyleText() {
    return `
      ...
      *::after {
      ...
      .power-circle-model-panel-scroll {
        width: 100%;
        height: 100%;
        min-height: 0;
        overflow-x: hidden;
      ...
      .power-circle-model-panel-scroll::-webkit-scrollbar {
        width: 0;
        height: 0;
      }
      .control-column {
      ...
      .layer-step-btn.active {
        border-color: rgba(250, 204, 21, 0.42) !important;
        background: rgba(250, 204, 21, 0.12) !important;
        color: #fef3c7 !important;
      }
      .layer-step-btn.active .layer-step-num {
        background: rgba(250, 204, 21, 0.9) !important;
        color: #111827 !important;
      }
      .geo-line-seg.proof-auxiliary-line {
      ...
      .geo-point-ring {
        fill: none !important;
        stroke: rgba(37, 99, 235, 0.26) !important;
        stroke-width: 2px !important;
        pointer-events: none !important;
      }
      .geo-point {
      ...
      .area-proof-hud.collapsed .area-proof-hud-header {
        border-bottom-color: transparent !important;
      }
      .area-proof-hud.collapsed .area-proof-hud-chip,
      .area-proof-hud.collapsed .area-proof-hud-body {
        display: none !important;
      }
      .area-proof-hud.collapsed .area-proof-hud-arrow {
      ...
      .area-proof-row.final {
        grid-template-columns: 1fr !important;
        justify-items: center !important;
        padding: 4px 0 6px !important;
      }
      .area-proof-shape-wrap {
        min-width: 0 !important;
        display: grid !important;
        justify-items: center !important;
        gap: 5px !important;
      }
      .area-proof-card-label {
        color: #0f172a !important;
        font-size: 11.5px !important;
        font-weight: 900 !important;
      ...
      .area-proof-html-shape.pc-pd-rect {
        border-color: #10b981 !important;
        background: rgba(16, 185, 129, 0.15) !important;
      }
      .area-proof-html-shape.pt-square {
        border-color: #ef4444 !important;
        background: rgba(239, 68, 68, 0.15) !important;
      }
      .area-proof-final-label {
      ...
      .area-proof-final-shape {
        ...
        background: rgba(239, 68, 68, 0.15) !important;
        outline: 2px dashed rgba(250, 204, 21, 0.76) !important;
...
${unifiedJmModelHudFinalOverrides()}`;
  ...
  function extractLayout(body, sourceStyle, cleanup) {
    ...
    const scene = makeShadowHost("power-circle-model-scene");
    ...
    const panel = makeShadowHost("power-circle-model-panel");
    const panelShell = document.createElement("div");
    panelShell.className = "power-circle-model-panel-scroll";
  ...
  function runSourceScript(script, sceneRoot, panelRoot, sceneHost, cleanup) {
    ...
    const runner = new Function(
      ...
      "clearInterval",
      "CSS",
      `"use strict";\n${source}\n//# sourceURL=jm_model_m13_source.js`
    );
    runner(
      scopedDocument,
      scopedWindow,
  ...
  window.MATH_VISUAL_SCENES[CARD_ID] = {
    async mount(container, context) {
      ...
      try {
        ...
        console.error("Failed to mount power of circle model card:", error);
